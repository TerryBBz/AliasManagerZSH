...
export class DataManager {
  ...
  async loadData() {
    ...
    try {
      ...
      // Si le fichier n'existe pas ou est invalide, créer une structure par défaut
      return {
        groups: {
          default: {}
        }
  ...
  validateData(data) {
    ...
    // S'assurer qu'il y a toujours un groupe par défaut
    ...
    for (const [groupName, aliases] of Object.entries(data.groups)) {
      ...
      for (const [aliasName, aliasData] of Object.entries(aliases)) {
        ...
        // S'assurer que chaque alias a les propriétés requises
        if (typeof aliasData.cmd !== 'string') {
          delete data.groups[groupName][aliasName];
          continue;
        }

        // Définir disabled par défaut à false si absent
  ...
  async importFromZshrc(zshrcPath = null) {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Détecter les sections basées sur les commentaires
        if (line.startsWith('# ───────────────────────────────────────────────────────────────────────────────')) {
        ...
        if (aliasMatch) {
          ...
          importedAliases[currentGroup][cleanName] = {
            cmd: cleanCommand,
            description: description || `Alias importé depuis .zshrc`,
            disabled: false
          };
        }
      }
  ...
  extractSectionName(commentLine) {
    ...
    if (commentLine.includes('UTILITAIRES') || commentLine.includes('DÉVELOPPEMENT')) return 'utils';
  ...
  cleanAliasCommand(command) {
    ...
    // Nettoyer les échappements de guillemets doubles dans les guillemets simples
  ...
  extractAliasDescription(lines, currentIndex) {
    // Chercher un commentaire inline après l'alias
  ...
  async updateSyncFileAndGitPush(action, details) {
    try {
      // Mettre à jour le fichier de sync avec les données actuelles
      const currentData = await this.loadData();
      ...
      console.log('⚠️  Erreur lors de la synchronisation Git:', error.message);
    }
  }
}